    ...
    public partial class frmPrincipal : Form
    {
        ...
        private void frmPrincipal_Load(object sender, EventArgs e)
        {
            ...
            for (int x = 0; x < ABotones.Length; x++)//Para trabajar con objeto boton 
            {
                ...
                {
                    ABotones[x].BOTON.Top = 2*k * (ABotones.Length - x);
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < ABotones.Length; i++)
            {
                for (int x = 0; x < i; x++)
                {
                    if (ABotones[i].BOTON.Bounds.IntersectsWith(ABotones[x].BOTON.Bounds))
                    {
        ...
        private void Colision(Botones a, Botones b)
        {
             ...
             * ListaB.CopyTo(ABotones) asi se copia la lista al arreglo
             * si eso es lo facil ahora no mas me falta comprender lo que has hecho
             * En mi cabeza suena bien, no lo he probado hahaha 
             * jajajajaja suele pasar :D
             * Pero si entiendes mi idea? o esta muy rara ?  :b
             *   Array.Resize(ref ABotones,10) Ejemplo redimensionar
                 */
            #endregion Conversasion
            if (a.BOTON.BackColor == b.BOTON.BackColor && a.VAL % 2 != 0 || b.VAL % 2 != 0)
            {
